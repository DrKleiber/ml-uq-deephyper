...
class StarDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.input_list[index][-7:-3] != self.output_list[index][-7:-3]:
        ...
        input_features = input_features[A_idx,16,:,:]   # cut one slice from the data

        output_qois = torch.load(self.output_list[index])
        output_qois = output_qois[0,16,:,:].unsqueeze(0)
